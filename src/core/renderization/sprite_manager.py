...
class SpriteManager:
    def __init__(self, user, data):
        ...
        self.static_sizeof = {
            'default': data['size']['default'], # will return an tuple
            'others': data['size']['others'] #will return an... thing... ((['things'], (values)), (['things'], (values)))
        }
        self.static = {
            'explore': {
    ...
    def image_update(self, path, key_words):
        ...
        if self.static_sizeof['others'] is not None:
            for nums, lists in self.static_sizeof['others']:
                for num, items in enumerate(lists):
                    if items[0] == key_words:
                        self.image = pygame.transform.scale(self.image, self.static_sizeof['others'][nums][1])
        else:
            self.image = pygame.transform.scale(self.image, self.static_sizeof['default'])
